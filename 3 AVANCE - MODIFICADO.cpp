...
void Alta()
{
    ...
    for (int i = 0; i < alta; i++)
    {
        ...
        for (int j = 0; j < i; j++)
        {
            if (Producto[i].numero == Producto[j].numero)
            {
                printf("El numero ya existe, por favor ingrese otro \n");
                scanf_s("%d", &Producto[i].numero);
            }
        }
...
void Lista()
{
    ...
    printf("%cDesea filtrar la informacion%c 1.-Si  2.-No \n", 168, 63);
    scanf_s("%d", &eleccion);
    if (eleccion == 1)
    {
        printf("Seleccione el modo en el que la desee filtrar \n1.-Clasificacion \n2.-Genero \n3.-Consola \n");
        scanf_s("%d", &filtro);
        switch (filtro)
        {
        case 1:
            cin.ignore();
            printf("Ingrese la clasificacion que desee ver \n");
            getline(cin, cadena);
            for (int i = 0; i < alta; i++)
            {
                if ((cadena == Producto[i].clas) && (Producto[i].status != "ELIMINADO"))
                {
                    printf("Numero: %d \n", Producto[i].numero);
        ...
        case 2:
            cin.ignore();
            printf("Ingrese el genero que desee ver \n");
            getline(cin, cadena);
            for (int i = 0; i < alta; i++)
            {
                if ((cadena == Producto[i].clas) && (Producto[i].status != "ELIMINADO"))
        ...
        case 3:
            cin.ignore();
            printf("Ingrese la consola que desee ver \n");
            getline(cin, cadena);
            for (int i = 0; i < alta; i++)
            {
                if ((cadena == Producto[i].clas) && (Producto[i].status != "ELIMINADO"))
